...
def CloumCheck():
    ...
    if num == BONAM:
        type(Key.ENTER)
        click(Pattern("2016-03-22_213848.png").similar(0.83).targetOffset(8,-1))
        type(Key.RIGHT)
        ...
        while r != TPNAM:
            ...
            click("save.png")
            wait(10)
            click(Pattern("click.png").similar(0.87))
            type(Key.LEFT)
            type(Key.LEFT)
    else:
...
# CSVファイルのインポート(
import csv
FILENAME = "C:\Program Files\SikuliX\BO_data_ForIMP_4.csv"
f = open(FILENAME,"r")
reader = csv.reader(f)
IMPLIST = []
for i in reader:
